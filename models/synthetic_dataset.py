...
class SyntheticDataset(Dataset):
    def __init__(self, split_set='train',
        use_height=False, augment=False):

        self.data_path = os.path.join(ROOT_DIR,
            'data/data_%s'%(split_set))
    ...
    def __getitem__(self, idx):
        ...
        assert(20000 < point_cloud.shape[0] < 50000)
        ...
        # ------------------------------- LABELS ------------------------------
        box3d_centers = np.zeros((1, 3))
        box3d_sizes = np.zeros((1, 3))
        ...
        if np.shape(bboxes)[0] > 0:
            ...
            angle_classes[0] = angle_class       # 27.3*np.pi/180 -> angle_class=1
            angle_residuals[0] = angle_residual  # angle_residual=-2.7*np.pi/180
            target_bboxes[0,:] = bbox[0:3]

        point_votes_mask = point_votes[:,0]   # bool
        point_votes = point_votes[:,1:4]      # dx,dy,dz
